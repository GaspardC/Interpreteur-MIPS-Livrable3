...
int
getRegisterValue(registre r, int numReg)
{
...
int
getRegisterValueByStr(registre r, char *regName)
{
...
int
setRegisterValue(registre r, int numReg, int value)
{
	if (r != NULL && r->reg != NULL && numReg >= 0 && numReg <= 34) {
		//printf("itoaa %s\n", itoa(value, 16));
...
getName(char *regName)
{
	...
	if (a == 34)
		...
		return "problème dans l'affichage du nom du registre";
